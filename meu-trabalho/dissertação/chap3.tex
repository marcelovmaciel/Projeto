...
agentes nos quais uma das questões tem \(\sigma\) próximo a zero. Se \(f_i(\theta)\)for
uma distribuição uniforme (U[0,1]) quanto mais vezes fazemos o sorteio dos
\(o_i\)(quanto maior o número de questões), pela Lei dos Grandes Números, mais
centrada a média aritmética vai ser do valor esperado da distribuição (0.5).
Isso significa que já na condição inicial os agentes seriam de centro. Para que
isso não ocorra associamos para cada agente uma distribuição Beta, com \(\alpha\)s e
\(\beta\)s entre 1.1 e 100. Destas distribuições sorteamos os valores de \(o_i\)
para cada questão, de forma que os \(o\)s de cada agente são correlacionados e
as distribuições são centradas em diferentes pontos do espaço, como ilustrado na
Figura \ref{fig:betas100}:
...
(estabeleço na condição inicial quais os vizinhos). Quanto à implementação,
...
(12.0889, 45.0556)]. O primeiro agente da população vai então ter parâmetros
\(\alpha,\beta = (1.1, 56.0444) \), o segundo \(\alpha,\beta=(23.0778,34.0667)\) e assim em
diante.

Voltando aos atributos dos agentes, o \(\sigma^2\) é uma variável global ; um
