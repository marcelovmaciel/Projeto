...
questão segundo as equações 2.3 e 2.4. Dado que os agentes agora tem um
...
alguma proporção de agentes tem uma questão no seu perfil \(I\) cujos \(\sigma\)
associados são aproximadamente zero. Com isso buscamos analisar o papel de
agentes ``inflexíveis'' na dinâmica populacional. A proporção de agentes
intransigentes ou inflexíveis é assim um parâmetro da simulação.
...
\item A proporção de agentes intransigentes \(0.0 \leq (\text{p\_intran}) \leq 0.5\);
...
depende de qual a rede os agent s estão. Uma determinada proporção de agentes,
...
seguimos \citeonline{ten2016sensitivity} e combinamos histogramas da medida do
