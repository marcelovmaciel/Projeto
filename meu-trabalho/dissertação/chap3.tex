...
alguma\footnote{ Uma outra implicação possível, a ser adicionada em trabalhos
  ...
  sentido dado por \citeonline{zaller1992simple}: a qual questão os agentes
...
Do ponto de vista metodológico, \citeonline{macy2015signal} argumentam que
...
Voltando aos atributos dos agentes, o \(\sigma^2\) é uma variável global de
...
\begin{figure}[h]
    ...
    usual de polarização\footnote{\citeonline{bramson2016disambiguation} discute
...
O tamanho amostral e o número de parametrizações não independem do método de
...
parâmetros. Segundo \citeonline[p.284]{saltelli2002making} esse número de
...
\); a mesma lógica é aplicada aos outros termos \footnote{ Para maiores detalhes
  do cálculo dos índices ver \citeonline[p.159-167]{saltelli2008global}.}. O
...
\begin{figure}[H]
  ...
  \caption{Evolução dos pontos ideais quando há ruído e agentes intransigentes.
    ...
    de \citeonline{martins2009bayesian}, adicionando agentes com opiniões em
    ...
    inspiração bayesiana de \citeonline{martins2009bayesian}, que por sua vez
    recupera um comportamento qualitativo similar ao modelo canônico de
    \citeonline{deffuant2000mixing}. Nosso modelo recupera também, de duas
    formas distintas, a convergência da maioria da população a um dos extremos
    encontrada por \citeonline{deffuant2002can} por meio tanto de intransigentes
    nos extremos, quanto de agentes não totalmente intransigentes distribuídos
    ao longo do espectro político.

    Um comportamento novo encontrado usando o modelo é a importância da
    ...
    \citeonline{flache2017}, modelos de dinâmicas de opinião levam naturalmente
