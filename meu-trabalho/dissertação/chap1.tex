...
Por fim, a relação de preferência fraca pode ser definida como:
...
%Quando o conjunto de alternativas é
%probabilístico o modelo considera a noção adicional de estados ( \(w \in W\)), 
%ações ( \( a \in A\)) e loterias : \(W\) é o conjunto de fatos fora do controle do ator que
%influenciam o resultado de sua decisão; ações do agente que levam a um
%determinado resultado; e loterias são funções \cite{sep-rationality-normative-utility}. 

%Tendo em vista a
%noção de estados, a teoria da escolha racional pressupõe que os agentes tem uma
%crença, uma distribuição de probabilidade, que atribui uma probabilidade
%\(p(w)\) para cada \(w \in W\).
