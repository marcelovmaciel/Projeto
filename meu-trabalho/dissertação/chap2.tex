...
\item AMBs muitas vezes não tem supervisores/controladores centrais;
  ...
  amplamente usados em OD \cite{castellano2012social,flache2017}. Essas
  propriedades são ditas emergentes na medida em que caracterizam o sistema e
  não seus componentes. O conceito, modernamente, é relativo tanto ao quadro
  conceitual quanto às expectativas dos pesquisadores sobre quais propriedades
  sistêmicas poderíamos deduzir a partir das propriedades das partes
  \cite{epstein2006generative}. Isto é, propriedades emergentes são aquelas que
  resultam da interação entre as partes do sistema, mas que não seriam a priori
  dedutíveis unicamente das propriedades delas. Em sistemas onde há emergência a
  conexão entre as propriedades sistêmicas e das partes é feita pela compreensão
  do papel da \textit{interação} entre as partes. Em sua forma mais simples
  propriedades emergentes condicionam indiretamente o comportamento dos agentes.
  Se os agentes tiverem consciência dessa propriedade sistêmica temos, contudo,
  um \textit{feedback} direto, ao invés de indireto, entre agentes e
  ...
  Já no modelo da Regra de Maioria a interação é : a cada unidade de tempo um
  grupo de tamanho \textit{r} é selecionado aleatoriamente e todos os agentes
  mudam sua opinião para a opinião da maioria do grupo
  ...
  \cite{galam2012sociophysics, galam1986majority}. O Modelo Sznajd também é
  bastante discutido na literatura \cite{sznajd2000opinion,
  ...
  convergência. Se eles discordarem, \(i\) adota a opinião do outro vizinho, e
  \(j\) faz o mesmo.

  \todo[inline,color=yellow!10]{\citeonline{stauffer2003convince} na descrição
    do modelo Sznajd realmente não menciona essa segunda regra, mas não discute
    porque. Eles falam em frustração, mas é outra coisa ( quando na atualização
    simultanea metade dos vizinhos sao +1 e a outra metade -1, no caso
    unidimensional um vizinho de cada lado).\citeonline{sirbu2017opinion} fala
    dessa segunda regra, e ela está na descrição do original; é para eu suprimir
  essa segunda regra na descrição do modelo?}
  ...
  diferenças\footnote{Um terceiro tipo de modelo elencado por
    \citeonline{flache2017}, além dos modelos de influência social assimilativa
