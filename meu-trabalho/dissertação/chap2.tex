...
compartilhadas e se reconhecem como membros dessa comunidade.
...
simulações que envolvem agentes discretos, onde agentes, seus atributos, e
possivelmente um ambiente são definidos algoritmicamente
\cite{sayama2015introduction} \footnote{Embora existam modelos baseados em
  agentes que historicamente não tenham sido diretamente implementados em
  computadores, como os modelos de Schelling e de Sakoda
  \cite{hegselmann2017thomas}, ABMs costumam ser implementados como simulações
  num computador.}. Num ABM existem três noções primitivas: os
...
\item AMBs muitas vezes não tem supervisores/controladores centrais;
  ...
  afetar a opinião de um agente \(i\) \cite{page2008uncertainty}. Podemos
  dividir a estrutura de interação numa \textit{topologia} de interação e numa
  \textit{regra de interação}. A topologia de interação define quais agentes
  estão conectados com \(i\), e podem, potencialmente, afetá-lo. A regra de
  interação define como \(i\) interage com os agentes desse conjunto(seus
  ...
  por vez, uma interação em diáde, ou com algum subconjunto de seus vizinhos.


  Mais precisamente, um modelo de
  dinâmica de opinião pode ser definido \footnote{Seguindo
    \citeonline{troitzsch2017axiomatic}}. como um conjunto \textbf{M} tal que
  \textbf{M} é igual a \{A, O, T, I, U\}, onde:
\begin{itemize}
